...
function waGetSourceCode() {
    ...
    if [[ ! -d "$SOURCE_PATH" ]]; then
        $SUDO git clone --recurse-submodules --remote-submodules https://github.com/Stumie/winapps_fedora_kinoite.git "$SOURCE_PATH"
    else
...
function waGetInquirer() {
    ...
    if [ -d "$SYS_SOURCE_PATH" ]; then
        ...
        curl -o "$INQUIRER" "https://raw.githubusercontent.com/Stumie/winapps_fedora_kinoite/main/install/inquirer.sh"
    fi
