...
function inqMenu() {
    ...
    # Convert options into the appropriate format for a 'kdialog' menu.
    ...
    SELECTED_OPTIONS_STRING=$(kdialog --menu "$DIALOG_TEXT" "${DIALOG_OPTIONS[@]}") || exit 0
...
function inqChkBx() {
    ...
    # Convert options into the appropriate format for a 'kdialog' checklist.
    for PADDED_OPTION in "${PADDED_OPTIONS[@]}"; do
    ...
    # Each option is enclosed in double quotes within the output string.
    # For example: '\"Option 1  \" \"The  Second Option   \" \"    Option Number 3 \"'
    SELECTED_OPTIONS_STRING=$(kdialog --checklist "$DIALOG_TEXT" "${DIALOG_OPTIONS[@]}") || exit 0
